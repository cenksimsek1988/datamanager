...
@Repository
public class RiskFactorService extends AbstractDataService<RiskFactor> {
	...
	public RiskFactor findRiskFactorByIndicatorRegionAndFrequency(Integer ind, Integer rgnId, String frq) {
		String sql = initQuery() + " where (region_id=" + rgnId + " and indicator_id=" + ind + " and frequency_code='" + frq + "')" ;
	...
	@Override
	public void save(RiskFactor rf) {
		RiskFactor in = findRiskFactorByIndicatorRegionAndFrequency(rf.getIndicatorCode(), rf.getRegionId(), rf.getFrequencyCode());
		if(in==null) {
			insert(rf);
		} else {
			update(rf);
...
//	}
	...
	public void deleteForRegionsAndSource(List<Region> regions, String src) {
		...
		String sql = getDeleteSql() + getWhereStatementForRegions(regions) +
				" and source_code='" + src + "'";
